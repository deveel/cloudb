	...
	public abstract class AbstractStore : IStore {
				...
				while (cur_pointer != -1 && inserted == false) {
					...
					if ((header & DeletedFlag /* 0x08000000000000000L */) == 0) {
						throw new ApplicationException("Assert failed - area not marked as deleted.  " +
					        ...
					        // The condition for inserting the area into bin chain
        //long area_size = header ^ 0x08000000000000000L;
        //if (area_size >= size || searches >= 12) {
        // This orders the free space by pointer reference so free nodes are
        // discovered first towards the start of the addressable space.
					        if (cur_pointer > pointer || searches >= 12) {
		...
		private long Alloc(long size) {
			...
			if (size < 0) {
			    ...
			    // Currently areas > 128GB can not be allocated
    if (size > (128L * 1024L * 1024L * 1024L)) {
      throw new IOException("Area allocation " + size + " too large.");
    }
			...
			if (found_bin_index == -1) {
				...
				total_allocated_space += real_alloc_size;
				      ...
				      // Check the pointer is within the first 60 bits
				      if (( ((ulong)working_pointer) & 0x0F000000000000000L ) != 0) {
        throw new IOException("Address space exhaustion.");
      }
				...
				total_allocated_space += real_alloc_size;
				      ...
				      // Check the pointer is within the first 60 bits
				      if (( ((ulong)free_area_pointer) & 0x0F000000000000000L ) != 0) {
        throw new IOException("Address space exhaustion.");
      }
		...
		private class StoreMutableArea : StoreArea, IMutableArea {
			...
			public void WriteInt4(int i) {
				ByteBuffer.WriteInt4(i, buffer, 0);
