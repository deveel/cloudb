		...
		public IJournalledResource CreateResource(String resource_name) {
			...
			lock (all_resources) {
				...
				if (resource == null) {
					...
					if (ENABLE_LOGGING) {
						resource = new Resource(this, resource_name, id, accessor, page_size);
					} else {
						resource = new NonLoggingResource(this, resource_name, id, accessor, page_size);
					}
		...
		private sealed class JournalFile {
			...
			internal void Persist(long start, long end) {
				if (js.Debug.IsInterestedIn(LogLevel.Information)) {
					js.Debug.Log(LogLevel.Information, this, "Persisting: " + file);
				}
				...
				while (!finished) {
					...
					if (type == 2) {       // Resource id tag
						...
						if (js.Debug.IsInterestedIn(LogLevel.Information)) {
							js.Debug.Log(LogLevel.Information, this, "Journal Command: Tag: " + id + " = " + resource_name);
						}
						...
						if (js.Debug.IsInterestedIn(LogLevel.Information)) {
							js.Debug.Log(LogLevel.Information, this, "Journal Command: Delete: " +
															   resource_name);
						}
						...
						if (js.Debug.IsInterestedIn(LogLevel.Information)) {
							js.Debug.Log(LogLevel.Information, this, "Journal Command: Set Size: " +
						...
						*/
							else if (type == 21) {   // Most Recent Page modification 
						long id = din.ReadInt64();
						long page = din.ReadInt64();
						int page_size = din.ReadInt32();
						int off = din.ReadInt32();
						int len = din.ReadInt32();
						...
						if (js.Debug.IsInterestedIn(LogLevel.Information)) {
							js.Debug.Log(LogLevel.Information, this,
									"Journal Command: Page Modify: " + resource_name +
					...
					} else if (type == 1) {   // Regression Page modification
						// We changed the format to include the page size.  Previously we
						// mostly used 8k page sized by default, so use that as the default.
						long id = din.ReadInt64();
						long page = din.ReadInt64();
						int page_size = 8192;
						int off = din.ReadInt32();
						...
						if (js.Debug.IsInterestedIn(LogLevel.Information)) {
							js.Debug.Log(LogLevel.Information, this, "Journal Command: Check Point.");
						}
				...
				for (int i = 0; i < sz; ++i) {
					...
					if (js.Debug.IsInterestedIn(LogLevel.Information)) {
						js.Debug.Log(LogLevel.Information, this, "Synch: " + r);
					}
					r.Synch();
				}
			...
			internal JournalEntry LogPageModification(String resource_name, long page_number, byte[] buf, int off, int len) {
				...
				lock (this) {
					...
					// Write the header
					long resource_id = v;
					data_out.Write((long)21);
					data_out.Write(8 + 8 + 4 + 4 + 4 + len);
					data_out.Write(resource_id);
					data_out.Write(page_number);
					data_out.Write(js.page_size);
					data_out.Write(off);
 ...
 */
			internal void BuildPage(long in_page_number, int in_page_size,
						   long position, byte[] buf, int off) {
				long type;
				long resource_id;
				long page_number;
				int page_size;
				...
				lock (this) {
					data.Read(position, buffer, 0, 8);
					type = ByteBuffer.ReadInt8(buffer, 0);
					if (type == 1) {
						data.Read(position + 8, buffer, 0, 28);
						resource_id = ByteBuffer.ReadInt8(buffer, 4);
						page_number = ByteBuffer.ReadInt8(buffer, 12);
						page_size = 8192;
						page_offset = ByteBuffer.ReadInt4(buffer, 20);
						page_length = ByteBuffer.ReadInt4(buffer, 24);
						data_start_offset = 36;
					} else if (type == 21) {
						data.Read(position + 8, buffer, 0, 32);
						resource_id = ByteBuffer.ReadInt8(buffer, 4);
						page_number = ByteBuffer.ReadInt8(buffer, 12);
						page_size = ByteBuffer.ReadInt4(buffer, 20);
						page_offset = ByteBuffer.ReadInt4(buffer, 24);
						page_length = ByteBuffer.ReadInt4(buffer, 28);
						data_start_offset = 40;
					} else {
						throw new IOException("Invalid page type. type = " + type +
											  " pos = " + position);
					}
					// Assert the page number and page size match, otherwise error.
					if (page_number != in_page_number) {
						throw new IOException("Page numbers do not match.");
					}
					if (page_size != in_page_size) {
						throw new IOException("Page sizes do not match.");
					}

					// Read the content.
					data.Read(position + data_start_offset,
									 buf, off + page_offset, page_length);
				}
 ...
 * The page size
 */
			protected readonly int page_size;
			...
			internal abstract void PersistPageChange(long page, int pageSize, int off, int len, BinaryReader din);
		...
		private sealed class NonLoggingResource : AbstractResource {
			...
			internal override void PersistPageChange(long page, int pageSize, int off, int len, BinaryReader din) {
				// No-op
			}
			...
			public override void Read(long page_number, byte[] buf, int off) {
				...
				try {
					...
					for (int i = 0; i < sz; ++i) {
						...
						lock (file) {
							file.BuildPage(page_number, page_size, position, buf, off);
						}
					}
					...
					if (to_process == null) {
						...
						for (int i = 0; i < sz; ++i) {
							...
							try {
								...
								js.Debug.Log(LogLevel.Error, this, "Error persisting journal: " + jf);
								js.Debug.Log(LogLevel.Error, e);
